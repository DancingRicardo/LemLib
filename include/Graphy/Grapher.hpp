...
class AsyncGrapher : public TaskWrapper {
    ...
    public:
         ...
         */
        AsyncGrapher(const std::string& title, const uint refreshRate);
         ...
         * @param rate refresh rate
         */
        void setRefreshRate(const uint refreshRate);
         ...
         */
        void insertNewGraph(std::shared_ptr<std::pair<std::string, std::vector<double>>> newGraph);
    protected:
        void loop() override;
};
